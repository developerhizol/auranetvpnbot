...
async def edit_main_menu(target, user_id: int, first_name: str, username: str = None):
    ...
    if is_new:
        ...
        gift_text = f"\n\n{emoji('5203996991054432397', '🎁')} <i>В честь первого захода мы подарили вам пробную подписку Family на 3 дня!</i>"
    else:
    ...
    if is_active:
        ...
        sub_status = "активна"
        end_date_str = format_date(subscription_end) if subscription_end else "—"
        plan = db.get_user_plan(user_id)
        plan_names = {"solo": "Solo", "premium": "Premium", "family": "Family"}
        plan_name = plan_names.get(plan, "Solo")
