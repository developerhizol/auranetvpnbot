...
def get_config_for_user(user_id: int, device_id: str = None, device_name: str = None, token: str = None) -> str:
    ...
    if is_subgram_need_subscribe:
        config_lines = [
            ...
            "#profile-update-interval: 1",
            "#hide-settings: 1",
            "#announce: ❌ Ваша подписка приостановлена",
            "",
    ...
    if plan == "premium":
        ...
        if sub_info:
            expire_date = sub_info.get('premium_until')
            if expire_date:
                expire_timestamp = get_expire_timestamp(expire_date)
    else:
        free_until = db.get_free_until(user_id)
        if free_until:
            expire_timestamp = get_expire_timestamp(free_until)
    
    if expire_timestamp > 0:
        config_lines.append(f"#subscription-userinfo: expire={expire_timestamp}")
    ...
    config_lines.append(f"#profile-web-page-url: https://t.me/streamnetvpnbot")
...
async def handle_sub(request):
    ...
    if is_browser(user_agent):
        ...
        # Если Free и подписка активна - проверяем спонсоров
        if plan == "free" and SUBGRAM_API_KEY:
            try:
                ...
                if subscriptions and subscriptions.get('status') == 'ok':
                    ...
                    if not all_subscribed and sponsors:
                        # Нужно подписаться - редирект на check.html
                        return web.HTTPFound(f"/sub/check/{token}")
            except Exception as e:
                logger.error(f"SubGram check error for user {user_id}: {e}")
