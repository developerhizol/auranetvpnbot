...
from handlers import start_router, about_service_router, payment_router, profile_router, admin_router
...
# Закодированный текст в base64 с префиксом для announce
ANNOUNCE_BASE64 = "base64:"
...
def get_profile_headers(user_id: int) -> list:
    headers = ["#profile-title: 🚀 aura net"]
    ...
    headers.append(f"#announce: {ANNOUNCE_BASE64}")
    headers.append("#profile-update-interval: 1")
    headers.append("")
...
async def handle_sub(request):
    ...
    # Если браузер - проверяем нужно ли подписаться на спонсоров
    if is_browser(user_agent):
        plan = db.get_user_plan(user_id)
        is_active = db.is_subscription_active(user_id)
        
        if plan == "free" and is_active and SUBGRAM_API_KEY:
            try:
                subscriptions = await subgram.get_user_subscriptions(user_id)
                if subscriptions and subscriptions.get('status') == 'ok':
                    sponsors = subscriptions.get('additional', {}).get('sponsors', [])
                    all_subscribed = all(s.get('status') == 'subscribed' for s in sponsors)
                    
                    if not all_subscribed and sponsors:
                        return web.HTTPFound(f"/check/{token}")
            except Exception as e:
                logger.error(f"SubGram check error for user {user_id}: {e}")
        
        return await serve_index()
    
    # Остальной код для мобильных приложений
    headers_dict = {}
...
async def handle_check_subscription(request):
    ...
    try:
        ...
        if not sponsors_data:
            return web.json_response({"error": "Failed to get sponsors"}, status=500)
        
        status = sponsors_data.get('status')
        
        if status == 'ok':
            db.extend_free_subscription(user_id, days=3)
...
async def handle_confirm_subscription(request):
    ...
    try:
        ...
        if not subscriptions:
            return web.json_response({"error": "Failed to check subscriptions"}, status=500)
        
        sponsors = subscriptions.get('additional', {}).get('sponsors', [])
        all_subscribed = all(s.get('status') == 'subscribed' for s in sponsors)
        
        if all_subscribed or not sponsors:
            db.extend_free_subscription(user_id, days=3)
            db.update_subgram_linked(user_id, 1)
            return web.json_response({
                "success": True,
...
async def get_banned_message() -> str:
    return f"{BAN_EMOJI} <b>Вы были заблокированы.</b>\n\nЕсли считаете, что ваш бан был необоснованным, свяжитесь с администрацией @AuraNetworkBot."
